  ...
  template: `
    ...
    @if (showAddForm()) {
      <div class="mb-6 rounded-xl border border-emerald-200 bg-emerald-50 p-5">
        <h3 class="mb-3 text-sm font-semibold text-gray-700">Novo cliente</h3>
        <div class="flex flex-col gap-3 sm:flex-row sm:flex-wrap">
          ...
          <div class="flex flex-1 flex-col gap-1" style="min-width: 220px">
            <label for="new-logo" class="text-xs font-medium text-gray-600">Logo URL (opcional)</label>
            ...
            <button type="button" class="flex items-center gap-4 text-left" (click)="onSelect(client.slug)">
              @if (client.logoUrl) {
                ...
                <div class="flex h-12 w-12 items-center justify-center rounded-lg bg-primary-50">
                  <lucide-icon [img]="BuildingIcon" [size]="24" class="text-primary-500" />
                </div>
              }
              <div class="flex flex-col gap-0.5">
                <span class="text-base font-semibold text-gray-800 transition-colors group-hover:text-primary-500">{{ client.name }}</span>
                <span class="text-sm text-gray-400">Gestão de Risco Ergonômico</span>
              </div>
            </button>
