  ...
  template: `
    ...
    @if (loading()) {
      ...
      <div class="flex flex-col items-center justify-center rounded-xl border border-dashed border-gray-300 py-20 text-center dark:border-gray-600">
        ...
        <p class="mt-3 text-sm font-medium text-gray-600 dark:text-gray-300">Nenhum dado AEP disponível</p>
...
export class ErgonomicsDiagnosis implements OnInit {
  ...
  copyShareLink(): void {
    const url = `${location.origin}/compartilhar/${this.ctx.clientSlug()}/${this.ctx.unitSlug()}/diagnostico-ergonomico`;
